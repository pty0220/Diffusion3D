...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        x=x[:,-1:,...]  #loss is only calculated on the last channel, not on the input brain A image
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
           ...
           for i in tqdm.tqdm(indices):
                ...
                # if i%50==0:
                #     result = np.squeeze(img.cpu()[0, -1,...])
                #     plt.imshow(result, cmap = 'gray')
                #     plt.savefig(os.path.join(result_path,'savefig'+str(i)+'.png'))
                #     np.save(os.path.join(result_path, 'sampling'+str(i)), result)
                with th.no_grad():
                    if img.shape != (img.shape[0], 2, img.shape[2], img.shape[3], img.shape[4]):
                        img = torch.cat((org_A,img), dim=1)       #in every step, make sure to concatenate the original image to the sampled segmentation mask
    ...
    def ddim_sample_loop_known(
        ...
        t = th.randint(100,101, (b,), device=device).long().to(device)
        noise = th.randn_like(img[:, :1, ...]).to(device)
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device)
        indices = list(range(time[0]-1))[::-1]
        org_A = img[:, :-1, ...] #original brain A image
        print("Start DDIM sampling")
        ...
        for i in tqdm.tqdm(indices):
                ...
                with th.no_grad():
                 if img.shape != (img.shape[0], 2, img.shape[2], img.shape[3], img.shape[4]):
                     img = torch.cat((org_A, img), dim=1).float()
    ...
    def training_losses_A2B(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        res = x_start[:, -1:, ...]
        res_t = self.q_sample(res, t, noise=noise)     #add noise to the CT image 
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float()
        terms = {}
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE,]:
                ...
                terms["vb"] = self._vb_terms_bpd(model=lambda *args, r=frozen_out: r, x_start=res, x_t=res_t, t=t,clip_denoised=False,)["output"]
